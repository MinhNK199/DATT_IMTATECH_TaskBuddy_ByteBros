      ...
      <header className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white shadow-lg">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center space-x-4">
            {!loading && user && (
              <>
                <div className="flex items-center space-x-2 bg-white/20 rounded-full py-1 px-3">
                  ...
                  <span className="font-medium">Xin chào, {user.displayName?.split(' ')[0]}</span>
                </div>
                <button
            ...
            {!loading && !user && (
              <>
                <Link to="/login" className="text-white hover:text-indigo-100 transition-colors">Đăng nhập</Link>
        ...
        {menuOpen && (
          <div className="md:hidden bg-indigo-700 overflow-hidden transition-all duration-200 ease-in-out">
            <nav className="flex flex-col px-4 py-2 space-y-2 animate-fadeIn">
              {!loading && user && (
                <>
                  <div className="flex items-center space-x-2 py-2">
                    ...
                    <span className="text-white font-medium">Xin chào, {user.displayName?.split(' ')[0]}</span>
                  </div>
                  <button
                    onClick={handleLogout}
              ...
              {!loading && !user && (
                <>
                  <Link to="/login" className="py-2 text-white hover:bg-indigo-600 rounded px-2 transition-colors">Đăng nhập</Link>
                  <Link to="/register" className="py-2 text-white hover:bg-indigo-600 rounded px-2 transition-colors">Đăng ký</Link>
                </>
              )}
            </nav>
          </div>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div>
              ...
              <p className="text-gray-300 text-sm">Giải pháp quản lý công việc cá nhân của bạn</p>
            </div>
            <div>
              ...
              <ul className="space-y-1 text-sm">
                ...
                <li><Link to="/tasks" className="text-gray-300 hover:text-white transition-colors">Nhiệm vụ</Link></li>
                <li><Link to="/profile" className="text-gray-300 hover:text-white transition-colors">Hồ sơ</Link></li>
              </ul>
            </div>
            <div>
          ...
          <div className="mt-8 pt-4 border-t border-gray-700 text-center text-sm text-gray-400">
            &copy; {new Date().getFullYear()} TaskBuddy. Đã đăng ký bản quyền.
          </div>
        </div>
      </footer>
